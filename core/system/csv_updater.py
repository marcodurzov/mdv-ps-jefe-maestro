...
def update_local_csv(game, df_norm):
    ...
    existing_df["_fecha_dt"] = pd.to_datetime(
        existing_df["FECHA"], dayfirst=True, errors="coerce"
    )

    last_date = existing_df["_fecha_dt"].max()

    df_norm["_fecha_dt"] = pd.to_datetime(
        df_norm["FECHA"], dayfirst=True, errors="coerce"
    )
