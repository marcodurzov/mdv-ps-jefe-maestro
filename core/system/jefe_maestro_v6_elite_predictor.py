...
MODEL_FILE_TPL      = os.path.join(*CACHE, “model_v8*{name}.joblib”)
STATS_FILE_TPL      = os.path.join(*CACHE, “stats_v8*{name}.joblib”)
...
*POT        = {n: float(os.getenv(f”POT*{n.upper()}”, “1.0”)) for n in LOTTERIES}
...
ncols = [f”N{i}” for i in range(1, k + 1)]
feats, labels = [], []
for *, (*, row) in enumerate(df.iloc[::-1].iterrows()):
try:
...
for name in WORKER_NAMES:
per_lot[name] = *ml_score(combos, name)
...
cs = np.array([float(np.mean([float(cd.get(f”{min(a,b)}*{max(a,b)}”, 1.))
